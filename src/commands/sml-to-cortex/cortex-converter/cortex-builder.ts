...
import { mapToSnowDim, mapToSnowMeasure, mapToSnowTimeDim } from "./cortex-mapper";
...
function addSnowDimsFromSmlDim(
  ...
  if (dim.is_degenerate) {
    dim.level_attributes.forEach((attribute) => addIfNotHidden(attribute, dimsAndColsToPass, "", msgs, attrUniqueNames, logger)
    );
    getAllLevels(dim).forEach((level) => {
      level.secondary_attributes?.forEach((secondary) => addIfNotHidden(secondary, dimsAndColsToPass, "", msgs, attrUniqueNames, logger)
      );
    });
  } else {
    ...
    roleplays.forEach((roleplay) => {
      dim.level_attributes.forEach((attribute) => addIfNotHidden(attribute, dimsAndColsToPass, roleplay, msgs, attrUniqueNames, logger)
      );
      getAllLevels(dim).forEach((level) => {
        level.secondary_attributes?.forEach((secondary) => addIfNotHidden(secondary, dimsAndColsToPass, roleplay, msgs, attrUniqueNames, logger)
        );
        level.aliases?.forEach((alias) => addIfNotHidden(alias, dimsAndColsToPass, roleplay, msgs, attrUniqueNames, logger)
        );
        level.metrics?.forEach((metric) => snowMeasures.push(
          mapToSnowMeasure(
            metric,
            mapDatasetsToDims,
