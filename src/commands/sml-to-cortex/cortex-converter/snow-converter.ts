...
import { getSMLModel, filterSMLForModel, listUsedModels } from "./cortex-sml-processor";
...
export function convertSmlModelToCortexModel(
  ...
  if (mapDatasetsToDims) datasetsToDimsMap = createMapDatasetsToDims(modelObjects, models);
  ...
  addMeasures(modelObjects, newTable, datasetsToDimsMap, attrUniqueNames, logger);

  addCalculations(modelObjects, newTable, datasetsToDimsMap, attrUniqueNames, logger);

  addDimensions(smlObjects, modelObjects, newTable, rolePlays, datasetsToDimsMap, attrUniqueNames, logger);
