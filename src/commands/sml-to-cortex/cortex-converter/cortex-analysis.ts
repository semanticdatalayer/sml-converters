...
import { fmtDimRef, isRegularRelationship, addToMapWithSet } from "./cortex-tools";
import { 
  dimFromName, 
  listAttributesInDim, 
  addReferencedDims, 
...
export function listTimeColumns(smlObjects: SmlConverterResult): Set<string> {
  ...
  smlObjects.datasets.forEach((dataset) => dataset.columns.filter(SMLDatasetTypeGuard.isSimpleColumn).forEach((column) => {
    if ([SMLColumnDataType.DateTime,
 ...
 */
export function listNumericColumns(smlObjects: SmlConverterResult): Set<string> {
  const cols = new Set<string>();
  smlObjects.datasets.forEach((dataset) => dataset.columns.filter(SMLDatasetTypeGuard.isSimpleColumn).forEach((column) => {
    if ([SMLColumnDataType.BigInt,
...
export function createMapDatasetsToDims(smlObjects: SmlConverterResult, models: Array<SMLModel>): Map<string, Set<string>> {
